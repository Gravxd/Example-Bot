...
client.on('message', message => { // if the bot detects a message being sent
  ...
  if(message.author.bot) return; //Ignoring bots because they might cause spam with the bot
  if(!message.guild) return; //Ignoring if message was not in a guild/server
  
// this command will return the ping of the bot
  if(message.content === '-ping') { // if that message that was sent says: '-ping' then continue
    message.channel.send(`Pong!`).then(msg => {
      msg.edit(`Message Edit Latency is: ${msg.createdTimestamp - message.createdTimestamp}ms \n API Latency is : ${client.ws.ping}ms`)
    }); // message.channel = the channel that the message was sent in. the bot will send 'Pong!' into that channel
  }
  
  
// this command will send the help embed
   else if(message.content === '-help') { // if that message was '-help', continue
   ...
   // this command will add a role to someone
   else if(message.content.startsWith('-adddogrole')) { // if the message someone has sent, begins with '-adddogrole' then continue
